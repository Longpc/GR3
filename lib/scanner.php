...
#echo "<br>var:".$var_name; echo " varkeys:";print_r($var_token[3]); echo " declarekeys:";print_r($var_declare->array_keys); echo " diff:"; print_r($array_key_diff); echo " |||";
	...
	// add function to output by trigger
	function addtriggerfunction($mainparent)
	{
		...
		$mainparent->title = "Userinput reaches sensitive string when function <i>{$this->function_obj->name}()</i> is called.";
	...
	// add a variable to the varlist with name, token string, comment,var line in code,id
			...
			if( is_array($this->tokens[$i]) )
			{
				...
				*************************/		
				if($token_name === T_VARIABLE)
				{
					...
					{
						$this->variable_scan($i, 0, 'eval', 'Userinput is used as dynamic function name. ');
					}
					// $$var = 
					else if( ($this->tokens[$i-1] === '$' || ($this->tokens[$i-1] === '{' && $this->tokens[$i-2] === '$')) && ($this->tokens[$i+1] === '=' || in_array($this->tokens[$i+1][0], Tokens::$T_ASSIGNMENT)) )
					{
						$this->variable_scan($i, $this->tokens[$i-1] === '{' ? 2 : 1, 'extract', 'Userinput is used to build the variable name. may have vulnerabilities.');
					}
					...
					&& ($this->tokens[$i+1] === '=' || in_array($this->tokens[$i+1][0], Tokens::$T_ASSIGNMENT)) )
					{
						...
						while( $newbraceopen !== 0 )
						{
							...
							{
								addError('Could not find closing param.', array_slice($this->tokens, $i-2, 10), $this->tokens[$i-2][2], $this->file_pointer);
								break;	
							}
						}
					...
					{	
						...
						if($this->tokens[$i+2][0] === T_ARRAY && $this->tokens[$i+3] === '(' && $this->tokens[$i+4] !== ')')
						{
							...
							&& $keyindex < MAX_ARRAY_ELEMENTS )
							{
								...
								{
									addError('Could not find closing param of array()', array_slice($this->tokens, $i, 10), $this->tokens[$i+2][2], $this->file_pointer);
									break;	
								}
							}
				...
				|| (in_array($token_name, Tokens::$T_XSS) &&  $_POST['vector'] == 'xss') )
				{			
					$class='';
					/*************************
							T_STRING			
					...
					*************************/	
					...
					{	
						if(!$this->already_scanned($i))
						{
							...
							if( ($has_vuln_parameters && ($parameter_has_userinput || $parameter_func_depend)) || isset($this->scan_functions[$token_value][3]) ) 
							{
								$vulnstart=$i;
								$vulnadd=1;
								...
								} else
								{
									if(empty($new_find->title))
										$new_find->title = 'Userinput reaches sensitive string. May be use for XSS Attack. Click help for know more!';
